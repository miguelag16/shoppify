...
class SegmentationProblem(util2.SearchProblem):
    ...
    def succAndCost(self, state):
        ...
        optionsList = []
        numStores = 0
        for store in self.products.keys():
            for category in self.products[store]:
                if translateCategory[category] == state[0][0]:
                    numStores += 1
                    for item in self.products[store][category]:
        ...
        for option in optionsList:
            ...
            if len(newState) == 2:
                if len(newState[1]) == 3:
                    transportationPrice += (1 - self.listLength/float(20))*(transportationConverter[('all three')] / float(60))*10
                else:
                    transportationPrice += (1 - self.listLength/float(20))*(transportationConverter[newState[1]] / float(60))*10
            heuristicCost = ranking[option[1]]
...
storeIntermediate = collections.defaultdict(list)
storeAvgCosts = collections.defaultdict(float)
for store in products:
    ...
    for category in products[store].keys():
        ...
        for item in products[store][category]:
            if 'quantity' in item:
                if len(item) > 2 and any(char.isdigit() for char in item['quantity']) and any(not char.isdigit() for char in item['quantity']):
                    ...
                    numItems += 1
#                else:
#                    totalCost += float(re.findall("\d+\.\d+", item['price'])[0])
#                numItems += 1
        avgCosts[translateCategory[category]].append((store, totalCost / float(numItems)))
    print store, count
    for category, storeList in avgCosts.iteritems():
        for i in range(len(storeList)):
            if storeList[i][0] == store:
                storeIntermediate[store].append(storeList[i][1])
for store in storeIntermediate.keys():
    storeAvgCosts[store] = numpy.mean(storeIntermediate[store])
sortedList = []
for store in storeAvgCosts.keys():
    sortedList = sorted(storeAvgCosts, key = lambda x:storeAvgCosts[x])
for i in range(len(sortedList)):
    ranking[sortedList[i]] = i + 1
print ranking
