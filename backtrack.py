...
def create_nqueens_csp(n = 8):
    ...
    for i in range(n):
        domain = [(i, y) for y in range(n)]
        csp.add_variable("C%i" % i, domain)

    variables = csp.variables
    for j in range(n): 
        for k in range(j + 1, n):
            csp.add_binary_factor(variables[j], variables[k], lambda q1, q2: q1[1] != q2[1] and abs(float(q2[1]-q1[1])/(q2[0]-q1[0])) != 1)
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
    ...
    def print_stats(self):
        ...
        """
        # for assin in self.allAssignments:
        #     for a, p in assin.iteritems():
        #         print (a, p.name, p.price, p.store, p.massPerDollar)

        # self.optimalAssignment is a dict with userRequests as keys and (userRequests,Product) tuples as assignments when domain is the same

        print '\n'
        for r, p in self.optimalAssignment.iteritems():
            print (r.genericItemName, p.name, p.price, p.quantity, p.store, p.massPerDollar)
        ...
        if not self.mcv:
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    count = sum(self.get_delta_weight(assignment, var, val) == 0 for val in self.domains[var])
                    if count > curMCV[1]:
                        curMCV = (var, count)
    ...
    def arc_consistency_check(self, var):
        ...
        while(len(queue) != 0):
            ...
            for Xk in self.csp.get_neighbor_vars(Xj): # for every neighbor, for every value in the domain of the neighbor try to find a value in Xj's domain that makes it consistent
                for Xkval in list(self.domains[Xk]): # when editing an iterable create a copy 

                    XkvalConsistent = False
                    for Xjval in self.domains[Xj]:

                        if self.csp.binaryFactors[Xj][Xk][Xjval][Xkval] != 0:
                            XkvalConsistent = True
                            break

                    if not XkvalConsistent or (self.csp.unaryFactors[Xk] and self.csp.unaryFactors[Xk][Xkval] == 0):
                    # if not XkvalConsistent or (not self.csp.unaryFactors[Xk] and not self.csp.unaryFactors[Xk][Xkval]):
                        self.domains[Xk].remove(Xkval)
                        queue.add(Xk) 

        # END_YOUR_CODE

def get_budget_variable(csp, name, variables, maxSum):
    """
    ...
    """
    result = ('get_sum', name, 'final_sum')
    resultDomain = [t for t in range(0, maxSum+1)]
    csp.add_variable(result, resultDomain)
    ...
    for i, X_i in enumerate(variables):
        ...
        if i == 0:
            auxDomain = [(0, y) for y in range(0, maxSum+1)]
            csp.add_variable(A_i, auxDomain)

            def initialization_factor(val, tup):
                return tup[1] == val
            csp.add_binary_factor(X_i, A_i, initialization_factor)
        else:
            auxDomain = [(x, y) for x in range(0, maxSum+1) for y in range(x, maxSum+1)]
            csp.add_variable(A_i, auxDomain)

            def consistency_factor(tupA, tupB):
                return tupB[0] == tupA[1]  
            csp.add_binary_factor(('get_sum', name, i-1), A_i, consistency_factor)

            def processing_factor(val, tup):
                return tup[1] == (tup[0] + val)
            csp.add_binary_factor(X_i, A_i, processing_factor) 

    # consistency between A_n and result
    # hacky: reuse A_i because of python's loose scope
    csp.add_binary_factor(A_i, result, lambda tup, total: total == tup[1])
    return result
    # END_YOUR_CODE
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                # If req is None, then cid will also be None, as no course is scheduled, so we check the CID assigned to the tuple
                # This factors checks to see whether the assigned cid is in a quarter that it was requested in, if it's been assigned.
                csp.add_unary_factor((req, quarter), \
                    lambda cid: (cid == None) or (quarter in req.quarters))
        # END_YOUR_CODE
    ...
    def add_unit_constraints(self, csp):
        ...
        for qtr in self.profile.quarters:
            courseVars = []
            for req in self.profile.requests:
                for cid in req.cids:
                    ...
                    unitDomain = [units for units in range(minUnits, maxUnits+1)]
                    unitDomain.append(0)

                    csp.add_variable((cid, qtr), unitDomain)
                    courseVars.append((cid, qtr))
                    csp.add_binary_factor((req, qtr), (cid, qtr), lambda req_cid, req_units: ((req_cid != cid) and req_units == 0) or (req_cid == cid and req_units > 0))

            result = get_sum_variable(csp, "add_unit_constraints for %s" % qtr, courseVars, self.profile.maxUnits)
            csp.add_unary_factor(result, lambda total: self.profile.minUnits <= total and total <= self.profile.maxUnits)
        # END_YOUR_CODE
